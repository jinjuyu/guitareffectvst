		  ...
		  public:
			...
			void buttonClicked(Button *button)
			{
				if(button == addMapping)
				{
					...
					popeye.addItem(1, L"Add Key Mapping");
					popeye.addItem(2, L"Add MIDI CC Mapping");
					popeye.addItem(3, L"Add Open Sound Control Mapping");

					switch(popeye.showAt(button))
					{
						case 1:
							{
								AlertWindow win(L"Keypress mapping",
												L"Enter the key combination to map this command to:\n\n",
												AlertWindow::NoIcon);

								win.addButton(L"OK", 1);
						...
						case 2:
							{
								AlertWindow win(L"MIDI CC mapping", 
												L"Select a MIDI CC to map this command to:",
												AlertWindow::NoIcon);

								win.addComboBox(L"midiCc", MidiMappingManager::getCCNames());
								...
								if(win.runModalLoop())
								{
									...
									index = win.getComboBoxComponent(L"midiCc")->getSelectedId();
									if(index)
									{
										String tempstr;
						...
						case 3:
							{
								AlertWindow win(L"Open Sound Control mapping", 
												L"Enter OSC address to map this command to:",
												AlertWindow::NoIcon);

								//win.addTextEditor(L"oscAddress", L"");
								win.addComboBox(L"oscAddress", oscManager->getReceivedAddresses());
								win.getComboBoxComponent(L"oscAddress")->setEditableText(true);
								...
								if(win.runModalLoop())
								{
									...
									//tempstr = win.getTextEditorContents(L"oscAddress");
									tempstr = win.getComboBoxComponent(L"oscAddress")->getText();
				...
				else
				{
					...
					popeye.addItem(1, L"Remove Mapping");
