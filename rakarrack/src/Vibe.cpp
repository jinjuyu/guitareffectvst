...
void
Vibe::processReplacing (float **inputs,
								float **outputs,
								int sampleFrames)
{
    ...
    for (i = 0; i < param->PERIOD; i++) {
        ...
        //Left Channel
        input = bjt_shape(fbl + inputs[0][i]);
...
//Orig code
            input = bjt_shape(fbr + inputs[0][i]);
            //Close Comment here if using Inline instead
...
// Close comment here if inlining
            ...
            outputs[0][i] = outl*fcross + outr*flrcross;
            outputs[1][i] = outr*fcross + outl*flrcross;
        }  else {  //if(Pstereo)
            outputs[0][i] = outl;
            outputs[1][i] = outl;
        }
